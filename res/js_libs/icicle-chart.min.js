...
function VisCreate() {
    ...
    function highlightCluster(clusters, strokeStyle, triggerQt = true) {
        d3.selectAll("rect").style("stroke", "#000").style("stroke-width", 0.2).style("stroke-dasharray", "none");
    ...
    function highlightClusterByNameAndLevel(solidClusters, dashedClusters, level, sendtoQT) {
        ...
        const allClusters = [...solidClusters, ...dashedClusters];
        const clusterNodes = allClusters
            .map(clusterName =>
                dataForChart.descendants().find(d => d.data.name === clusterName && d.depth === level)
            )
        ...
        if (clusterNodes.length > 0) {
            clusterNodes.forEach(d => {
                d.descendants().forEach(descendant => {
                    ...
                    if (!selectedClusterContainer.includes(descendant.data.name)) {
                        selectedClusterContainer.push(descendant.data.name);
                    }

                    const strokeStyle =
                        solidClusters.includes(d.data.name) ? "none" : "4,4";
            ...
            //console.log(`Clusters with level ${level} not found.`);
        }
    }
    function changeAllStrokedLinesToSolid() {
        d3.selectAll("rect")
            .style("stroke-dasharray", "none");
    }
    function clicked(d) {
    ...
    //here we can mark
    //highlightClusterByNameAndLevel("No Agreement", 3); //cluster level bottom
    //highlightClusterByNameAndLevel("L6 CT", 2);//subclass level middle
    //highlightClusterByNameAndLevel("Glutamatergic", 1); //class level top
    //highlightClusterByNameAndLevel("All", 0); //all
    // Example usage:
    //highlightClusterByNameAndLevel (["No Agreement"],["Pvalb_1"], 3); // Solid line
    // highlightClusterByNameAndLevel ("L6 CT", 2); // Dashed line
    if (setMarkingsFlag) {
        highlightClusterByNameAndLevel(fullMark, partialMark, 3, false)
    }
}
